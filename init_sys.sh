...
function redisInstall()
    { 
       ...
       # sed -i '$a PATH=/usr/local/redis/bin:$PATH' /etc/profile && source /etc/profile
        ...
        groupadd rediser
        useradd -s /sbin/nologin -g rediser rediser
        ln -s /usr/local/redis/bin/redis-server /usr/bin/redis-server
        ln -s /usr/local/redis/bin/redis-cli /usr/bin/redis-cli
        mkdir -p /home/rediser/var/run
        chown -R rediser:rediser /home/rediser/var
        chown -R rediser:rediser /usr/local/redis/bin/redis-server
...
function nginxInstall()
    {   
        ...
        cp -rf ${theFiledir}/nginx/nginx-1.9.11.tar.gz /opt/fastdfs
        cp -rf ${theFiledir}/nginx/nginx /etc/init.d/
        yum install -y pcre pcre-devel zlib zlib-devel openssl openssl-devel
        cd /opt && unzip master.zip
        cd /opt/fastdfs && tar -xvf nginx-1.9.11.tar.gz
        cd nginx-1.9.11/
        ./configure\
        ...
        mkdir -p /data/fdfs/storage/group0
       # mkdir -p /data/fdfs/storage/group0/M00
       
        #cp -rf ${theFiledir}/fdfs/fastdfs.conf /etc/nginx/conf.d/fastdfs.conf
       ...
       # sed -i 's' /etc/fdfs/mod_fastdfs.conf
...
#tracker_server=10.148.0.8:22122
#tracker_server=10.148.0.10:22122
#
#
